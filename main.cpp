...
auto generate_header ( const std::vector < std::uint8_t >& in_buffer , int padding ) -> std::string
{
	...
	header.append ( std::to_string ( padding ) );
	header.append ( "\n\n#include <vector>\n" );
	header.append ( "#include <cstdint>\n\n" );
	header.append ( "inline std::vector <std::uint8_t> image = \n" );
	header.append ( "{" );
...
auto main ( int argument_count , char** argument_array ) -> int
{
	if ( argument_count < 3 )
	{
		printf ( "usage: simpleaes.exe binary_name.exe" );
		return -1;
	}
